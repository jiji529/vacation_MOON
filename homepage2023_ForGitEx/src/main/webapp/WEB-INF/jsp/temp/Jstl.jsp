...
<ul>
	<c:choose>
		...
		<c:when test="${step eq '6'}">
			<li>
				<h3>c:forEach Tag : 기본 반복 태그, 고정된 횟수 또는 초과 수집동안 중첩된 본문 내용을 반복</h3>
				<h4>기본문법 : <br/>
		...
		<c:when test="${step eq '9'}">
			<li>
				<h3>fn:contain() : 주어진 문자열이 특정 문자열을 포함하고 있는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '10'}">
			<li>
				<h3>fn:indexOf() : 주어진 문자열에서 특정 문자열이 어느 인덱스부터 시작하는지 확인하는데 사용</h3>
		...
		<c:when test="${step eq '11'}">
			<li>
				...
				<c:out value="String : Welcome to JSP programming"/><br/>
		...
		<c:when test="${step eq '12'}">
			<li>
				<h3>fn:endsWith() : 주어진 문자열이 특정 문자열 값으로 시작되는지 확인하는데 사용</h3>
				<c:out value="String : Welcome to JSP programming"/><br/>
				<c:set var="String" value="Welcome to JSP programming"/>
		...
		<c:when test="${step eq '17'}">
			<li>
				<h3>fn:substring() : 주어진 시작 및 끝 위치에 따라 문자열의 하위집합을 반환</h3>
